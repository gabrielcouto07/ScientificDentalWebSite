  ...
  return (
    <div className="grid gap-5 lg:grid-cols-2">
      {groups.map((g, gi) => (
        <section
          key={g.title}
          aria-labelledby={`spec-${slugify(g.title)}`}
          className={cn("card reveal p-5 sm:p-6", gi % 2 === 1 && "reveal-delay-1")}
        >
          <h3 id={`spec-${slugify(g.title)}`} className="flex items-center gap-2.5 text-base font-semibold">
          ...
          <dl className="mt-4 divide-y divide-escala">
            {g.items.map((item, itemIndex) => (
              <div key={`${itemIndex}-${item.label}`} className="grid gap-1 py-3 sm:grid-cols-[11rem_1fr] sm:gap-6">
